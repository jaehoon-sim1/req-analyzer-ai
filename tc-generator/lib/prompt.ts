...
### ⚠️ 핵심 원칙: 1건의 TC = 1개 절차 + 1개 기대결과
- 하나의 TestCase에는 **하나의 테스트 절차**와 **하나의 기대결과**만 작성합니다.
- 절차가 2개 이상이면 **별도 TC로 분리**하세요.
- 기대결과에 "1.", "2." 등 번호를 나열하지 마세요. 하나의 결과만 작성합니다.
- 세부 항목이 있으면 하이픈(-)으로 나열할 수 있지만, 번호 매기지 않습니다.

### Procedure (테스트 절차)
- 하나의 동작만 작성: "1. [메뉴경로] 메뉴를 선택한다." (하나의 TC에 하나의 절차)
...
- 하나의 결과만 작성 (번호 없이 또는 "1."만 사용)
- 예: "1. 약품정보 페이지가 노출된다."
- 예: "1. 아래 구성이 제공된다.\\n- '컨텐츠 유형' 타이틀 문구\\n- [유형 선택] 버튼\\n- [초기화] 버튼"
- 동사는 한국어 정중체: ~노출된다, ~제공된다, ~표시된다, ~반영된다, ~활성화된다
- **절대로 "1. ...", "2. ..." 형태로 여러 결과를 나열하지 마세요. 분리해서 각각의 TC로 만드세요.**

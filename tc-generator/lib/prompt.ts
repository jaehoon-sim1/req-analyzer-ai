...
10. **다중 페이지 입력**: 입력이 [페이지명] 구분자로 나뉜 다중 페이지일 수 있습니다. 페이지 순서에 관계없이 전체 맥락을 파악하여 논리적 흐름(환경설정 → 화면 구성 → 기능 동작 → 예외 처리)에 맞게 TC를 정리하세요. 각 페이지의 내용을 누락 없이 반영하되, 중복 내용은 하나로 통합하세요.
